 ...
 */
//TODO
#define DemMaxNumberEventEntryPrimary 5
                ...
                if((eventStatusBuffer[EventId-1].eventStatus & DEM_UDS_STATUS_TFTOC) == 0 )
                {
                        ...
                        if( (eventStatusBuffer[EventId-1].eventStatus | DEM_UDS_STATUS_CDTC) != 0  )
                        {
                            eventStatusBuffer[EventId-1].occurrence++;
                                         ...
                                         /*--------------Figure 7.21----------------*/
                eventStatusBuffer[EventId-1].eventStatus |= (DEM_UDS_STATUS_TF | DEM_UDS_STATUS_TFTOC | DEM_UDS_STATUS_TFSLC | DEM_UDS_STATUS_TFTOC);
            ...
            if( operationCycleStateList[eventStatusBuffer[EventId-1].EventParamterRef->DemOperationCycleRef_ptr->DemOperationCycleId] == DEM_CYCLE_STATE_START)
            {
                if(eventStatusBuffer[EventId-1].EventParamterRef->DemEnableConditionGroupRef_ptr->DemEnableConditionRef_ptr->DemEnableConditionStatus == TRUE)
                {
                    ...
                    if( eventStatusBuffer[EventId-1].errorStatusChanged == TRUE )
                    {
                        ...
                        storeEventEvtMem(eventParam, &eventStatusBuffer[EventId-1]);
                        Return_Value = E_OK;
 ...
 ***************************************************************/
Std_ReturnType Dem_CheckRangeOfDtcSelection (uint8 ClientId)///
{
     /* !LINKSTO SWS_Dem_00370,1 */
  Std_ReturnType Result = E_NOT_OK;

#if (DEM_DEV_ERROR_DETECT == STD_ON)
  if (Dem_ModuleState != MODULE_INTAILIZED)
  {
    /* !LINKSTO SWS_Dem_00124,1 */
  ...
  {
    /* !LINKSTO SWS_Dem_01252,1 */
    //DEM_REPORT_ERROR(ServiceId, DEM_E_WRONG_CONFIGURATION);
  }
  /* check if a DTC is selected for this client */
  else if (Dem_DTCSelection[ClientId].SelectionStatus < DEM_SELECTED_DTC)
  {
    /* !LINKSTO SWS_Dem_01255,1 */
    //DEM_REPORT_ERROR(ServiceId, DEM_E_WRONG_CONDITION);
  }
#endif

  
  if (Dem_DTCSelection[ClientId].SelectionStatus >= DEM_SELECTED_DTC)
...
{
     Std_ReturnType Result = DEM_WRONG_DTC;
  Result = Dem_CheckRangeOfDtcSelection( ClientId);
  if (Result == E_OK)
  {
    Result = E_OK ;//Dem_ProcessSelectDTC(ClientId, FALSE, FALSE);
    switch (Result)
    {
      case E_OK:
        /* Update the selection status to OK as the selection is valid */
        Dem_DTCSelection[ClientId].SelectionStatus = DEM_SELECTED_CHK_OK;
        break;
      case DEM_WRONG_DTC:
        /* Update the selection status to wrong DTC */
        Dem_DTCSelection[ClientId].SelectionStatus = DEM_SELECTED_CHK_WRONG_DTC;
        break;
      case DEM_WRONG_DTCORIGIN:
        /* Update the selection status to wrong origin */
        Dem_DTCSelection[ClientId].SelectionStatus = DEM_SELECTED_CHK_WRONG_ORIGIN;
        break;
      /* CHECK: NOPARSE */
    ...
    /* DTC selection is successful and valid */
    if (Dem_DTCSelection[ClientId].SelectionStatus >= DEM_SELECTED_CHK_OK)
    {
      Dem_DTCSelection[ClientId].DTCInfo.RecordNumber = RecordNumber;
      /* set the next record number to search from the beginning if all
       * freeze frame records are requested */
      Dem_DTCSelection[ClientId].DTCInfo.NextRecNum =
       //(RecordNumber == DEM_ALL_RECORD_NUM) ? DEM_FIRST_FF_RECORD_NUM :
         (RecordNumber == 0xff) ? 0x01 :RecordNumber;

      /* Freeze frame selection is successful */
      Dem_DTCSelection[ClientId].SelectionStatus = DEM_SELECTED_FFD;
    }
    else
    {
      /* Next freeze frame record number remains to the maximum freeze frame
       * record value */
    }
  }
  return Result;
...
Std_ReturnType Dem_SelectExtendedDataRecord( uint8 ClientId, uint8 ExtendedDataNumber)
{
  ...
  if (Result == E_OK)
  {
    ...
    if (Dem_DTCSelection[ClientId].SelectionStatus >= DEM_SELECTED_CHK_OK)
    {
      Dem_DTCSelection[ClientId].DTCInfo.RecordNumber = ExtendedDataNumber;
      /* set the next record number to search from the beginning if all extended
       * records are requested. The search begins from 0x90 if all OBD records
       * are requested */
      Dem_DTCSelection[ClientId].DTCInfo.NextRecNum =
       ( (ExtendedDataNumber == DEM_ALL_RECORD_NUM) ? DEM_FIRST_ED_RECORD_NUM :
       ( (ExtendedDataNumber == DEM_ALL_OBD_EDR) ? DEM_FIRST_OBD_EDR :
          ExtendedDataNumber) );

      /* Extended Data Record selection is successful */
      Dem_DTCSelection[ClientId].SelectionStatus = DEM_SELECTED_EDR;
    }
    else
    {
      /* Result remains DEM_WRONG_DTC */
    }
  }
  return Result;
}
...
********************************************************************************************/
Std_ReturnType Dem_SetFreezeFrameRecordFilter
( uint8 ClientId, Dem_DTCFormatType DTCFormat, uint16* NumberOfFilteredRecords )
{
Std_ReturnType Result = E_NOT_OK;
  ...
  else if (NumberOfFilteredRecords == NULL)
  {
    ...
    /* !LINKSTO Dem.FilterForRecords.API,3 */
    ...
    /* variable indicating if service is in use by other client*/
    boolean isServiceBlocked = TRUE;
    /*
     * ENTER critical section
     */
    DEM_ENTER_EXCLUSIVE_AREA();
    if ( (Dem_RecordFilterLockByClientId == DEM_CLIENT_ID_INVALID) ||
         (Dem_RecordFilterLockByClientId == ClientId)
       )
    {
      Dem_RecordFilterLockByClientId = ClientId;
...
#if (DEM_NUM_OF_PIDS_IN_OBDFF > 0U)
      Dem_ObdFFConsidered = FALSE;
#endif
#if (DEM_DEV_FREEZE_FRAME_USED == STD_ON)
#if ((DEM_MAX_NUMBER_DEVFFA_ENTRY > 0U) || (DEM_MAX_NUMBER_DEVFFB_ENTRY > 0U))
      Dem_DevFFConsidered[DEM_DEVFF_A] = FALSE;
      Dem_DevFFConsidered[DEM_DEVFF_B] = FALSE;
#endif
#endif

/* !LINKSTO Dem_OBD_0001,2 */
#if (DEM_OBD_Support == STD_ON)
      Dem_RecordFilterFormat = DTCFormat;
#else
      Dem_RecordFilterFormat = DEM_DTC_FORMAT_UDS;
#endif
    ...
    {
      DEM_REPORT_ERROR(DEM_SID_SetFreezeFrameRecordFilter, DEM_E_WRONG_CONDITION);
    }
#endif
  }
